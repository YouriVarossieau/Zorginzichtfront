...
function showPolicy(policy) {
  ...
  document.getElementById("kosten").innerHTML = `${parseValue(
    policy.costs
  )} per maand`;
  ...
  policy.additional_insurances.map((add) => {
    test += `
    	...
    	<td>  
		${parseValue(add.costs)} per maand 
		</td>
<tr><td colspan="2"> <div style="background-color:white"> Uitgebreide dekking voor uw ${
  ...
  let baseIns = `	<tr>
		<td>Basis</td>
		<td>${parseValue(policy.costs)}</td>
	</tr>`;
  let totalCosts = `Totale premie: ${parseValue(totalCostsAdd + policy.costs)}`;
